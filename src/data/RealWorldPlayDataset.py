...
from util import get_image_to_tensor_balanced, dict_to_tensor, repeat_interleave
...
class RealWorldPlayDataset(torch.utils.data.Dataset):
    ...
    def process_cams(self, images, depths=None):
        ...
        for image in images:
            ...
            image_t = self.image_to_tensor(image).unsqueeze(0)
        ...
        images_tensor = torch.squeeze(images_tensor, dim=1) # (N, C, H, W)
    ...
    def process_sample(self, sample):
        ...
        # action_orig = self.process_action(sample['action'])
        ...
        action = torch.Tensor(sample["actions"])
        ...
        tcp_pose = get_tcp_pose(sample["actions"][:3],
                                sample["actions"][3:7])  # (T, 4, 4) or (4, 4)
        ...
        if "static_intrinsics" in self.cam_info:
            intrin = self.cam_info["static_intrinsics"]
            pose = self.cam_info["static_extrinsic_calibration"]

            scale_x, scale_y = 1, 1
            if images[0].shape[-2:] != self.image_size:
                scale_x, scale_y = self.image_size[0] / images_shps[0][0], self.image_size[1] / \
                                images_shps[0][1]
            for intr in intrin:
                cx.append(intr["cx"] * scale_x)
                cy.append(intr["cy"] * scale_y)
                fx.append(intr["fx"] * scale_x)
                fy.append(intr["fy"] * scale_y)
        ...
        if "gripper_intrinsics" in self.cam_info and "static_intrinsics" in self.cam_info:
            ...
            poses = torch.cat([gripper_poses, poses], dim=0)
    ...
    def __getitem__(self, index):
        ...
        sample_path = episode[idx_in_ep] if extra_frames == 0 else episode[
